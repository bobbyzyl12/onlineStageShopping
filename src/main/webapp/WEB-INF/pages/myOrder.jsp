	...
	<div style="margin-bottom:20px;">
		<table>
	 		<tr>
  				<th>订单号</th>
  				<th>总价</th>
  				<th width=50px>总期数</th>
  				<th>状态</th>
  				<th width=50px>本期期数</th>
  				<th width=75px>本期金额</th>
  				<th width=80px style="padding:30px;">时限</th>
  				<th>支付</th>
  			</tr>
	 	<c:forEach items="${myOrderList}"  var="myOrder">
	 		<tr>
	 			<td>${myOrder.orderID}</td>
	 			<td>￥${myOrder.orderTotalPrice}</td>
	 			<td>${myOrder.totalStageNum}</td>
	 			<td>
	 				<c:choose>
    					<c:when test="${myOrder.orderState == '1'}">
       						<p style="color:#00b300;font-weight:bold;">等待支付</p>
   						</c:when>
    					<c:when test="${myOrder.orderState=='7'}">
       						<p style="color:#red;font-weight:bold;">已冻结</p>
   						</c:when>
    					<c:when test="${myOrder.orderState=='2'}">
       						<p style="color:#a64dff;font-weight:bold;">等待审核</p>
   						</c:when>
   						<c:when test="${myOrder.orderState=='4'}">
       						<p style="color:#e5e600;font-weight:bold;">未通过审核</p>
   						</c:when>
   						<c:when test="${myOrder.orderState=='5'}">
       						<p style="color: #4d4dff;font-weight:bold;">支付完成</p>
   						</c:when>
   						<c:when test="${myOrder.orderState=='0'}">
       						<p style="color:red;font-weight:bold;">已逾期</p>
   						</c:when>
					</c:choose>
				</td>
	 			<td>${myOrder.nextNo}</td>
	 			<td>￥${myOrder.nextStagePrice}</td>
	 			<td>
	 				<fmt:formatDate value="${myOrder.deadline}" pattern="yyyy-MM-dd"/>
	 			</td>
	 			<td>
					<a href="${ctx}/page/jumpToOrderDetail?orderID=${myOrder.orderID}">
						<input class="btn2" id="pay" type="button" value="订单详情"/>
					</a>
	 			</td>
	 		</tr>
	 	</c:forEach>
